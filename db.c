...
inline int64_t low_bin_nb_arithmetic(int64_t* data, int64_t size, int64_t target)
{
  ...
  */
  ...
  while (left < right) {
    mid=left+((right-left)>>1); // Calculate the midpoint without using division
    
    int64_t diff=data[mid]-target;
    int64_t mask=(diff>>63)|(!diff&1); // Mask calculation to update left/right
...
inline void low_bin_nb_4x(int64_t* data, int64_t size, int64_t* targets, int64_t* right)
{
  ...
  */
  ...
  while (left[0] < right[0] || left[1] < right[1] || left[2] < right[2] || left[3] < right[3]) {
    for (int i = 0; i < 4; ++i) {
      mid[i]=left[i]+((right[i]-left[i])>>1); // Calculate midpoint for each search

      int64_t diff=data[mid[i]]-targets[i];
      int64_t mask=(diff>>63)|(!diff&1); // Mask calculation to update left/right
...
inline void low_bin_nb_simd(int64_t* data, int64_t size, __m256i target, __m256i* result)
{
  ...
  */
  ...
  while (!_mm256_testc_si256(_mm256_cmpgt_epi64(target, aright), _mm256_cmpgt_epi64(aleft, aright))) {
    __m256i amid = _mm256_add_epi64(aleft, _mm256_srai_epi64(_mm256_sub_epi64(aright, aleft), 1));
    __m256i cmp = _mm256_cmpgt_epi64(target, amid);
    aleft = _mm256_blendv_epi8(amid, _mm256_add_epi64(amid, _mm256_set1_epi64x(1)), cmp);
    aright = _mm256_blendv_epi8(aright, amid, cmp);
  }
...
int64_t band_join(int64_t* inner, int64_t inner_size, int64_t* outer, int64_t outer_size, int64_t* inner_results, int64_t* outer_results, int64_t result_size, int64_t bound)
{
  ...
  */
  ...
  int64_t count=0; // Variable to track the number of output results

  for (int64_t i=0; i < outer_size; ++i) {
    int64_t outer_value=outer[i];

    // Find the lower and upper bounds within the inner table
    int64_t lower_bound=outer_value-bound;
    int64_t upper_bound=outer_value+bound;

    // Binary search for lower and upper bounds in the inner table
    int64_t lower_index=low_bin_nb_mask(inner,inner_size,lower_bound);
    int64_t upper_index=low_bin_nb_mask(inner,inner_size,upper_bound);

    // Process matches within the bounds and store results
    for (int64_t j=lower_index;j<upper_index;++j) {
      if (count<result_size) {
        inner_results[count]=j;
        outer_results[count]=i;
        count++;
      } else {
        // If the number of results exceeds the allocated size, return the current count
        return count;
      }
    }
  }
...
int64_t band_join_simd(int64_t* inner, int64_t inner_size, int64_t* outer, int64_t outer_size, int64_t* inner_results, int64_t* outer_results, int64_t result_size, int64_t bound)
{
  ...
  */
  ...
  // Buffer to store 4 lower bound search results
  int64_t lower_bounds[4];

  for (int64_t i=0; i<outer_size; ++i) {
    int64_t outer_value=outer[i];

    // Find 4 lower bounds in the inner table using SIMD
    __m256i outer_simd=_mm256_set1_epi64x(outer_value);
    low_bin_nb_simd(inner,inner_size,outer_simd,(__m256i*)lower_bounds);

    // Process matches within the bounds and store results
    for (int64_t j=0; j<4; ++j) {
      int64_t lower_index=lower_bounds[j];
      int64_t upper_bound=outer_value+bound;

      // Perform sequential scan within the bounds
      while (lower_index<inner_size&&inner[lower_index]<=upper_bound) {
        if (count<result_size) {
          inner_results[count]=lower_index;
          outer_results[count]=i;
          count++;
        } else {
          // If the number of results exceeds the allocated size, return the current count
...
main(int argc, char *argv[])
{
	 ...
	 if (argc >= 5)
	   ...
	   {
	     fprintf(stderr, "Usage: db inner_size outer_size result_size bound <repeats>\n");
	     exit(EXIT_FAILURE);
	   }
